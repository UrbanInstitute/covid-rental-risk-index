...
final_levels_map1 <- c("Improvement (1 to 1.5)","Improvement (0.5 to 1)","Improvement (0 to 0.5)","No Change","Worse (0 to 0.5)","Worse (0.5 to 1)")

# housing map
ggplot() + 
  geom_sf(my_counties, mapping = aes(), fill = "#dfdfdf", color = "#ffffff", size = 1) + 
  coord_sf(datum = NA) + 
  geom_sf(countymap_direction %>% filter(!is.na(housing_avg_diff)), mapping = aes(fill=housing_avg_diff %>% cut(breaks = c(-1.5,-1,-0.5,0,0.0001,0.5,1), include.lowest = T)), color = "#696969", size = .01) + 
  coord_sf(datum = NA) + 
  scale_fill_manual(values = c(palette_urbn_cyan[7], palette_urbn_cyan[5],palette_urbn_cyan[3], "#ffffff", palette_urbn_red[3],palette_urbn_red[5]), labels = final_levels_map1) + 
...
ggsave("housing_improv_worse.png")
